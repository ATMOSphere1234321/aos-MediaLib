...
public class ShowTags extends VideoTags {
    ...
    long showId = -1;
    String newCover, newBackdrop, newNetworkLogo, newActorPhoto, newClearLogo, newStudioLogo;
    ...
    public long save(Context context, long videoId) {
        ...
        if (!showFound || baseInfoChanged) {
            ...
            values.put(ScraperStore.Show.SPOKENLANGUAGES_FORMATTED, getSpokenlanguagesFormatted());
            values.put(ScraperStore.Show.COUNTRIES_FORMATTED, getCountriesFormatted());
            ...
            values.put(ScraperStore.Show.NETWORKS_FORMATTED, getNetworksFormatted());
        ...
        if (!showFound) {
            ...
            log.debug("Inserting studios, networks, directors, writers, taglines, producers, screenplays, musiccomposer, spokenlanguage, countries seasonplots, actors, genres.");
            ...
            for (String network : mNetworks) {
                cop = ContentProviderOperation.newInsert(ScraperStore.Network.URI.SHOW);
                cop.withValue(ScraperStore.Show.Network.NAME, network);
                cop.withValue(ScraperStore.Show.Network.SHOW, showIdString);
                allOperations.add(cop.build());
            }
            ...
            for (String producer : mProducers) {
                cop = ContentProviderOperation.newInsert(ScraperStore.Producer.URI.SHOW);
                cop.withValue(ScraperStore.Show.Producer.NAME, producer);
                cop.withValue(ScraperStore.Show.Producer.SHOW, showIdString);
                allOperations.add(cop.build());
            }
            ...
            for (String spokenlanguage : mSpokenlanguages) {
                cop = ContentProviderOperation.newInsert(ScraperStore.Spokenlanguage.URI.SHOW);
                cop.withValue(ScraperStore.Show.Spokenlanguage.NAME, spokenlanguage);
                cop.withValue(ScraperStore.Show.Spokenlanguage.SHOW, showIdString);
                allOperations.add(cop.build());
            }

            for (String country : mCountries) {
                cop = ContentProviderOperation.newInsert(ScraperStore.Country.URI.SHOW);
                cop.withValue(ScraperStore.Show.Country.NAME, country);
                cop.withValue(ScraperStore.Show.Country.SHOW, showIdString);
                allOperations.add(cop.build());
            }
    ...
    private void updateInfo(String selection, String[] selectionArgs, ContentResolver contentResolver) {
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                ...
                log.debug("updateInfo: show found in db: storedBD " + storedBD + ", newBackdrop " + newBackdrop);
                log.debug("updateInfo: show found in db: storedNL " + storedBD + ", newNetworkLogo " + newNetworkLogo);
                log.debug("updateInfo: show found in db: storedNL " + storedBD + ", newActorPhoto " + newActorPhoto);
                log.debug("updateInfo: show found in db: storedNL " + storedBD + ", newClearLogo " + newClearLogo);
                log.debug("updateInfo: show found in db: storedNL " + storedBD + ", newStudioLogo " + newStudioLogo);
