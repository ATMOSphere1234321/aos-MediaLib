...
public class MovieTags extends VideoTags {
    ...
    public long save(Context context, long videoId) {
        ...
        values.put(ScraperStore.Movie.SCREENPLAYS_FORMATTED, getScreenplaysFormatted());
        values.put(ScraperStore.Movie.MUSICCOMPOSERS_FORMATTED, getMusiccomposersFormatted());
        values.put(ScraperStore.Movie.SPOKENLANGUAGES_FORMATTED, getSpokenlanguagesFormatted());
        ...
        for(String producer: mProducers) {
            cop = ContentProviderOperation.newInsert(ScraperStore.Producer.URI.MOVIE);
            cop.withValue(ScraperStore.Movie.Producer.NAME, producer);
            cop.withValueBackReference(ScraperStore.Movie.Producer.MOVIE, 0);
            allOperations.add(cop.build());
        }

        for(String screenplay: mScreenplays) {
            cop = ContentProviderOperation.newInsert(ScraperStore.Screenplay.URI.MOVIE);
            cop.withValue(ScraperStore.Movie.Screenplay.NAME, screenplay);
            cop.withValueBackReference(ScraperStore.Movie.Screenplay.MOVIE, 0);
            allOperations.add(cop.build());
        }

        for(String musiccomposer: mMusiccomposers) {
            cop = ContentProviderOperation.newInsert(ScraperStore.Musiccomposer.URI.MOVIE);
            cop.withValue(ScraperStore.Movie.Musiccomposer.NAME, musiccomposer);
            cop.withValueBackReference(ScraperStore.Movie.Musiccomposer.MOVIE, 0);
            allOperations.add(cop.build());
        }

        for(String spokenlanguage: mSpokenlanguages) {
            cop = ContentProviderOperation.newInsert(ScraperStore.Spokenlanguage.URI.MOVIE);
            cop.withValue(ScraperStore.Movie.Spokenlanguage.NAME, spokenlanguage);
            cop.withValueBackReference(ScraperStore.Movie.Spokenlanguage.MOVIE, 0);
            allOperations.add(cop.build());
        }

        for(String country: mCountries) {
            cop = ContentProviderOperation.newInsert(ScraperStore.Country.URI.MOVIE);
            cop.withValue(ScraperStore.Movie.Country.NAME, country);
            cop.withValueBackReference(ScraperStore.Movie.Country.MOVIE, 0);
            allOperations.add(cop.build());
        }
